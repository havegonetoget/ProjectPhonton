...
This project requires the use of a either a virual or local machine to run. A virtual machine can be found and downloaded from the internet with instructions. Gradle is used to connection to the database. To install gradle, first homebrew needs to be installed on you linux machine. Go to https://brew.sh/ and follow the instructions to get it set up properly. Then once you make sure homebrew gets installed correctly, run brew install gradle which will install everything needed to run the program including java. Once this project's repoistory is copied to the desired machine and it located in a folder, the user can go through the terminal and run the command ./gradle.bash to activate the main of the program. Must run python generator script (must run python script first) and main script spereately in two terminals. 
